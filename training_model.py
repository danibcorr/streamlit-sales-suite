...
def training_valid_loop(model: tf.keras.Model, optimizer: tf.keras.optimizers.Optimizer, class_weights: tf.Tensor,
...
def class_weights_calculation(labels: np.ndarray, y_train: np.ndarray) -> tf.Tensor:
    ...
    class_weights = compute_class_weight(class_weight='balanced', classes=np.unique(labels), y=np.argmax(y_train, axis=-1))
...
def main(dataset_path: str, dict_labels_path: str = "./models/trained_model/dict_labels.pkl") -> None:
    ...
    scheduled_lrs = warmupcosine_scheduler(X_train, cg.PARAMS["Batch size"], cg.PARAMS["Num epochs"], cg.LR_SCHEDULE["warmup_rate"],
                                            cg.LR_SCHEDULE["lr_start"], cg.LR_SCHEDULE["lr_max"])
