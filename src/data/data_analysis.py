...
def translation(year: int, year1: int = 0, year2: int = 0) -> dict:
    """
    Returns a dictionary of translations for a given year and optional year1 and year2.

    Args:
        year (int): The year for which translations are needed.
        year1 (int, optional): The first year for comparison. Defaults to 0.
        year2 (int, optional): The second year for comparison. Defaults to 0.

    Returns:
        dict: A dictionary containing translations for English and Spanish languages.
    """

    # Define a dictionary to store translations for different languages
    translations = {
        # Figure titles for bar charts
        "fig_bars_titles_1": f"Quantity sold per month in {year}",
        "fig_bars_titles_2": f"Quantity sold by category in {year}",
        "fig_bars_titles_3": f"Money made per month in {year}",
        # Figure titles for heatmaps
        "fig_heatmaps_titles_1": f"State/country ratio {year}",
        "fig_heatmaps_titles_2": f"Gender/state ratio {year}",
        ...
        "title_heatmap_1": [
            f"State/country ratio in {year1}",
            f"State/country ratio in {year2}",
        ],
        "title_heatmap_2": [
            f"Gender/State Ratio in {year1}",
            f"Gender/State Ratio in {year2}",
        ],
        "xaxis_title": "Month",
...
def quantity_sold_monthly(data: pd.DataFrame, year: int) -> plotly.graph_objects.Figure:
    ...
    fig = px.bar(
        data_year,
        x=x_axis.astype(str),
        y=num_sales_per_month,
        template="plotly_dark",
        text=num_sales_per_month,
    )
...
def product_category(data: pd.DataFrame, year: int) -> plotly.graph_objects.Figure:
    ...
    fig = px.bar(
        data,
        x=categories.astype(str),
        y=num_products_sold,
        template="plotly_dark",
        text=num_products_sold,
    )
...
def data_pipeline(df: pd.DataFrame, path: str, year: int, language: str) -> None:
    """
    ...
    fig_bars = make_subplots(
        ...
        subplot_titles=(
            (
                translator.translate(translations["fig_bars_titles_1"])
                if language != "English"
                else translations["fig_bars_titles_1"]
            ),
            None,
            (
                translator.translate(translations["fig_bars_titles_2"])
                if language != "English"
                else translations["fig_bars_titles_2"]
            ),
            None,
            (
                translator.translate(translations["fig_bars_titles_3"])
                if language != "English"
                else translations["fig_bars_titles_3"]
            ),
        ),
        specs=[
    ...
    fig_heatmaps = make_subplots(
        ...
        subplot_titles=(
            (
                translator.translate(translations["fig_heatmaps_titles_1"])
                if language != "English"
                else translations["fig_heatmaps_titles_1"]
            ),
            (
                translator.translate(translations["fig_heatmaps_titles_2"])
                if language != "English"
                else translations["fig_heatmaps_titles_2"]
            ),
        ),
    ...
    fig_bars.update_layout(
        ...
        width=None,
        title_text=(
            translator.translate(translations["fig_bars_first_title"])
            if language != "English"
            else translations["fig_bars_first_title"]
        ),
        # Center the title
        title_x=0.5,
        font=dict(size=14),
...
def compare_years(data: pd.DataFrame, year1: int, year2: int, language: str) -> None:
    """
    ...
    fig_overlapped_quantity.add_bar(
        name=f"{year1}",
        x=fig_year1_quantity["data"][0]["x"],
        y=fig_year1_quantity["data"][0]["y"],
        marker_color=COLOR_BAR_1,
    )
    fig_overlapped_quantity.add_bar(
        name=f"{year2}",
        x=fig_year2_quantity["data"][0]["x"],
        y=fig_year2_quantity["data"][0]["y"],
        marker_color=COLOR_BAR_2,
    )

    fig_overlapped_revenue = go.Figure()
    fig_overlapped_revenue.add_bar(
        name=f"{year1}",
        x=fig_year1_revenue["data"][0]["x"],
        y=fig_year1_revenue["data"][0]["y"],
        marker_color=COLOR_BAR_1,
    )
    fig_overlapped_revenue.add_bar(
        name=f"{year2}",
        x=fig_year2_revenue["data"][0]["x"],
        y=fig_year2_revenue["data"][0]["y"],
        marker_color=COLOR_BAR_2,
    )

    fig_overlapped_categories = go.Figure()
    fig_overlapped_categories.add_bar(
        name=f"{year1}",
        x=fig_year1_categories["data"][0]["x"],
        y=fig_year1_categories["data"][0]["y"],
        marker_color=COLOR_BAR_1,
    )
    fig_overlapped_categories.add_bar(
        name=f"{year2}",
        x=fig_year2_categories["data"][0]["x"],
        y=fig_year2_categories["data"][0]["y"],
        marker_color=COLOR_BAR_2,
    )

    # Add titles
    fig_overlapped_quantity.update_layout(
        title_text=(
            translator.translate(translations["title_quantity"])
            if language != "English"
            else translations["title_quantity"]
        ),
        xaxis_title=(
            translator.translate(translations["xaxis_title"])
            if language != "English"
            else translations["xaxis_title"]
        ),
        yaxis_title=(
            translator.translate(translations["yaxis_title"])
            if language != "English"
            else translations["yaxis_title"]
        ),
        legend_title=(
            translator.translate(translations["legend_title"])
            if language != "English"
            else translations["legend_title"]
        ),
    )

    fig_overlapped_revenue.update_layout(
        title_text=(
            translator.translate(translations["title_revenue"])
            if language != "English"
            else translations["title_revenue"]
        ),
        xaxis_title=(
            translator.translate(translations["xaxis_title"])
            if language != "English"
        ...
        yaxis_title=(
            translator.translate(translations["yaxis_title_2"])
            if language != "English"
            else translations["yaxis_title_2"]
        ),
        legend_title=(
            translator.translate(translations["legend_title"])
            if language != "English"
    ...
    fig_overlapped_categories.update_layout(
        title_text=(
            translator.translate(translations["title_categories"])
            if language != "English"
            else translations["title_categories"]
        ),
        xaxis_title=(
            translator.translate(translations["xaxis_title_2"])
            if language != "English"
            else translations["xaxis_title_2"]
        ),
        yaxis_title=(
            translator.translate(translations["yaxis_title"])
            if language != "English"
    ...
    fig_heatmaps_1 = make_subplots(
        rows=2,
        cols=2,
        subplot_titles=(
            translator.translate(translations["title_heatmap_1"])
            if language != "English"
            else translations["title_heatmap_1"]
        ),
    ...
    fig_heatmaps_2 = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=(
            translator.translate(translations["title_heatmap_2"])
            if language != "English"
            else translations["title_heatmap_2"]
        ),
        specs=[[{}, {}]],
        vertical_spacing=0.25,
        horizontal_spacing=0.25,
