...
def load_model(model_path: str, num_classes: int) -> tf.keras.Model:
    ...
    model = build_model(dropout = cg.PARAMS["Dropout MLP"], 
...
def image_processing(image: Image, col: st.columns) -> tf.Tensor:
    ...
    caption_title = 'Image loaded.' if language in ['English', 'Inglés'] else 'Imagen cargada.'
...
def make_prediction(model: tf.keras.Model, labels: dict, file: str, col2: st.columns) -> str:
